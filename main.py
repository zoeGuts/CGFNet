...
parser = argparse.ArgumentParser(description='Global-Group Attention Network.')

parser.add_argument('-d', '--device_ids', type=int, default=1, metavar='GPU', 
                    help="number of gpus to use")
parser.add_argument('--method', type=str, default='clam', help='method (options:base, cbam, clam, ours)')
...
def main():
    ...
    assert args.method in ['base', 'cbam', 'clam', 'ours'], "method must be the one of ['base', 'cbam', 'clam', 'ours',]. "
    ...
    data_root = r'./datasets/aerial_images/'
    if args.dataset == 'ERA':
